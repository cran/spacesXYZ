...
It is a $C^\infty$ curve in a 2D chromaticity space, usually either xy (1931) or uv (1960).
Since this package does not deal with spectra, 
we use a good approximation of the locus instead.
At this time only one approximation of the reference locus is available -
a spline (using \code{stats::splinefun()} with \code{method="fmm"})
...
but I am sure Robertson would approve of it.
It has $C^1$ continuity and is in very good agreement with the \emph{reference locus}.
The maximum RMS error in the uv-plane is about $7.3 \times 10^{-6}$
over the temperature range from 1667 to $\infty$ K.
...
and we use \code{stats::splinefun()} with \code{deriv=1} to calculate them.
...
The Robertson parameterization is \emph{not} $C^1$,
but the \emph{geometric continuity} class is $G^1$.
The McCamy parameterization is $C^1$.
For the Robertson and native isotherms the valid temperature range is 1667 to $\infty$ K.
For the McCamy isotherms the valid temperature range is 1702 to 34539 K.
...
as seen in Figure 1 near 2000 K.
...
a circle (in red) centered at the point and passing through the locus at $T$ = 6500 K.
...
@
It is apparent that the curvature of the locus near $T$ = 6500 K is fairly
...
all the intermediate isotherms intersect at that same point,
see Figure 2(3.11) in \cite{Wyszecki1982}.
Let's calculate and plot these 30 points.
<<fig4, echo=TRUE, message=TRUE, fig.pos="H", fig.height=6, out.width='1.0\\linewidth', fig.cap='Adjacent pairs of Robertson isotherms intersect below the locus.' >>=
mired = seq( 0, 600, by=10 )
temp  = 1.e6/mired
uv = planckLocus( temp, param='native' )    # a spline curve
par( omi=c(0,0,0,0), mai=c(0.5,0.5,0.1,0.1) )
...
off = planckLocus( temp, param='robertson', delta=-0.05 ) - top
center = matrix( 0, length(temp)-1, 2 )
...
The evolute of the \emph{reference locus} (defined by spectra) is a continuous curve.
The evolute of the spline locus is a continuous curve, except at 30 jumps.
