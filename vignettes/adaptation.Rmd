...
I find chromatic adaptation interesting mathematically.
...
\tag{1.1}
1)&  \hspace{5pt} I:I = J:J  \\
2)&  \hspace{5pt}  A:I = B:I    \hspace{20pt}   \text{if and only if}  \hspace{20pt}   A = B
\end{align}
In axiom 2) the first equality is equality of appearance, and the second is equality of vectors.
...
By axiom 2) the _target color_ solution $X$ is unique.
...
Since $I:I = J:J$ by axiom 1), we can say that $T(I)=J$ exactly.
...
A              &= T_{JI}(T_{IJ}(A))      \hspace{32pt} \text{by axiom 2 in (1.1)}  \\
